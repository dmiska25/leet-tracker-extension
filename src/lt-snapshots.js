...
(() => {
  ...
  if (!LT) {
    console.error(
      "[LeetTracker] LT namespace missing; ensure lt-global.js loads before lt-snapshots.js"
    );
    return;
  }

  const { GRAPHQL_URL, DAY_S } = LT.consts || {};
  ...
  async function detectCurrentLanguage(code, problemSlug = null) {
    try {
      const { userId } = await LT.net.getUserInfoWithCache();
      const problemId = problemSlug
        ? await LT.lcdb.getProblemIdFromSlug(problemSlug)
        : null;
      if (problemId && userId) {
  ...
  async function getCurrentCode() {
    ...
    try {
      const { problemSlug, problemId } = await LT.lcdb.getCurrentProblemId();
      if (problemId) {
        const currentLang = await detectCurrentLanguage("", problemSlug);
        const leetcodeCode = await LT.lcdb.getCodeFromLeetCodeDB(
          problemId,
          currentLang
        );
        if (leetcodeCode) {
  ...
  async function takeCodeSnapshot(username, problemSlug) {
    return LT.locks.withSnapshotLock(username, problemSlug, async () => {
      // Get current code
      ...
      let lastFinalCode = "";
      try {
        const snapshotData = await LT.dbInstance.getSnapshots(
          username,
          problemSlug
        );
      ...
      try {
        await LT.dbInstance.storeSnapshots(username, problemSlug, snapshotData);
      } catch (error) {
        console.warn(
  ...
  async function cacheTemplatesForProblem(problemSlug) {
    ...
    try {
      const idbTemplates = await LT.dbInstance.getTemplates(problemSlug);
    ...
    const templatesKey = LT.keys.getTemplatesKey
      ? LT.keys.getTemplatesKey(problemSlug)
      : LT.keys.templates
      ? LT.keys.templates(problemSlug)
      : `leettracker_templates_${problemSlug}`;

    const storageCached = await LT.store.get(templatesKey, null);
    ...
    try {
      ...
      if (templates.length > 0) {
        try {
          await LT.dbInstance.storeTemplates(problemSlug, templates);
        } catch (indexError) {
          console.warn(
            "[LeetTracker] IndexedDB store failed, using chrome.storage fallback:",
            indexError
          );
          await LT.store.set(templatesKey, {
            templates,
            timestamp: Date.now(),
            problemSlug,
          });
  ...
  async function handleFreshStartReset(username, problemSlug, currentCode) {
    return LT.locks.withSnapshotLock(username, problemSlug, async () => {
      ...
      // recordProblemVisit: prefer LT.watch if present; otherwise minimal fallback
      if (LT.watch && typeof LT.watch.recordProblemVisit === "function") {
        LT.watch.recordProblemVisit(username, problemSlug);
      } else {
        // Fallback: append to visitLog for last-24h
        try {
          const visitLogKey = LT.keys.getVisitLogKey
            ? LT.keys.getVisitLogKey(username)
            : LT.keys.visitLog
            ? LT.keys.visitLog(username)
            : `leettracker_problem_visit_log_${username}`;
          const nowSec = Math.floor(Date.now() / 1000);
          const log = (await LT.store.get(visitLogKey, [])) || [];
          log.push({ slug: problemSlug, ts: nowSec });
          const trimmed = log.filter((e) => nowSec - e.ts <= DAY_S);
          await LT.store.set(visitLogKey, trimmed);
        } catch {
          // ignore
        }
      }
      ...
      try {
        await LT.dbInstance.storeSnapshots(username, problemSlug, {
          snapshots: [],
          lastFinalCode: null,
        });
        return true;
  ...
  LT.snap = {
    ...
    cacheTemplatesForProblem,
    checkForFreshStart,
    handleFreshStartReset,
    startFreshStartWatcher,
  };
})();
