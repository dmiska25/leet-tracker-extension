...
(() => {
  ...
  if (!LT) {
    console.error(
      "[LeetTracker] LT namespace missing; ensure lt-global.js loads before lt-ingest.js"
    );
    return;
  }

  const { SYNC_LOCK_KEY, HEARTBEAT_TIMEOUT_MS, DAY_S } = LT.consts || {};
  const {
  ...
  const { get: getFromStorage, set: saveToStorage } = LT.store || {};
  ...
  async function loadSnapshotsIfApplicable(sub, username) {
    ...
    try {
      return await LT.dbInstance.getSnapshots(username, sub.titleSlug);
    } catch (error) {
      console.warn(
  ...
  async function buildRunEventsForSubmission(sub, username, startCandidatesMs) {
    ...
    const runs = await LT.dbInstance.getRunEventsInWindow(
      username,
      sub.titleSlug,
      startMs,
      endMs
  ...
  async function storeRecentJourney(username, submission) {
    ...
    try {
      await LT.dbInstance.storeJourneyArchive(username, submission);
      console.log(
  ...
  async function storeRecentRunGroup(username, submission, runEvents) {
    ...
    try {
      ...
      await LT.dbInstance.storeRunGroupArchive(username, submission);
